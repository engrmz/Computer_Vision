...
'''
Problem:	Use an ResNet50 encoder and a decoder to image to image reconstruction. It uses real object (i.e. cat, dog, etc.) instead of numbers.
...
class ImageReconstruction_Resnet50(nn.Module):
    def __init__(self):
        super(ImageReconstruction_Resnet50, self).__init__()
        # Input: N x 3 x 32 x 32
        model_resent50 = models.resnet50(pretrained=False)
        self.encoder_resent50 = torch.nn.Sequential(*(list(model_resent50.children())[:-1]))   # N x 2048 x 1 x 1
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(2048, 1024, 4, stride=1, padding=0),  # N x 1024 x 04 x 04
            nn.ReLU(),
            nn.ConvTranspose2d(1024, 512, 1, stride=1, padding=0),  # N x 512 x 04 x 04
            nn.ReLU(),

            nn.ConvTranspose2d(512, 256, 4, stride=2, padding=1),  # N x 256 x 08 x 08
            nn.ReLU(),
            nn.ConvTranspose2d(256, 128, 1, stride=1, padding=0),  # N x 128 x 08 x 08
            nn.ReLU(),

            nn.ConvTranspose2d(128, 64, 4, stride=2, padding=1),  # N x 64 x 16 x 16
            nn.ReLU(),
            nn.ConvTranspose2d(64, 32, 1, stride=1, padding=0),  # N x 32 x 16 x 16
            nn.ReLU(),

            nn.ConvTranspose2d(32, 16, 4, stride=2, padding=1),  # N x 16 x 32 x 32
            nn.ReLU(),
            nn.ConvTranspose2d(16, 3, 1, stride=1, padding=0),  # N x 3 x 32 x 32   <== OUTPUT
            nn.Sigmoid()
        )

    def forward(self, x):
        x = self.encoder_resent50(x)
...
def main():
    ...
    model = ImageReconstruction_Resnet50().to(device)

    # TODO - Ignore last classification layer of resent
    # TODO - use your decoder for image-reconstruction
